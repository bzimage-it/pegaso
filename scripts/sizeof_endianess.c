...
#define CR() printf("\n");
#define OFFSET(ident,offset) ( (unsigned char) (((unsigned char *)&ident)[offset] ))
...
#define ENDIANESS(type) do { \
	    ...
	    case 4: \
		x = (type) 0x01020304U; \
		printf("%02x %02x %02x %02x             | ", OFFSET(x,0) , OFFSET(x,1), OFFSET(x,2) , OFFSET(x,3)); \
	    ...
	    case 2: \
		x = (type) 0x0102U; \
		printf("%02x %02x                   | ", OFFSET(x,0) , OFFSET(x,1) ); \
		ENDIANESS_PRINT( IS_EQ(x,0,0x2) && IS_EQ(x,1,0x1) , \
				 IS_EQ(x,0,0x1) && IS_EQ(x,1,0x2) ); \
		break; \
	    case 8: \
	    	x = (type) 0x0102030405060708UL; \
		printf("%02x %02x %02x %02x %02x %02x %02x %02x | ", OFFSET(x,0) , OFFSET(x,1) , OFFSET(x,2) , OFFSET(x,3), OFFSET(x,4) , OFFSET(x,5) , OFFSET(x,6) , OFFSET(x,7) ); \
